#!/bin/sh
# run.sh -- build (if needed) and launch 5 Card Stud in the FujiNet-patched
...
# last built.
if [ ! -f 5card.rom ] || [ -n "$(find . -maxdepth 1 -name '*.bas' -newer 5card.rom)" ]; then
    echo "Building 5card.rom..."
    make
fi
