#!/usr/bin/env python3
"""Convert the MS-DOS CGA 2bpp tile set to Coleco Adam TMS9918 1bpp assets.

Reads  src/msdos/charset.h  (8x8 tiles, 16 bytes each: 2 bytes/row, 4px/byte,
CGA colors 0=black 1=green-background 2=red 3=white) and writes:

  src/adam/font.bin  768 bytes: the Namco arcade font, glyphs 0x20-0x7F,
                     1bpp, bit set where the CGA pixel is ink (color 0).
  src/adam/udg.h     UDG pattern table for codepoints 0x80+, 8 bytes/glyph,
                     consumed via IOCTL_GENCON_SET_UDGS. Foreground/background
                     colors are applied per cell at draw time, so each tile is
                     reduced to ink (bit=1) vs paper (bit=0) with a per-array
                     ink predicate (see TILE_MAP below).

Outputs are checked in; rerun from the repo root after art changes:

  python3 support/adam/convert-tiles.py
"""
...
FONT_BIN = REPO / "src/adam/font.bin"
UDG_H = REPO / "src/adam/udg.h"
...
# Codepoints are chosen so src/adam/graphics.c can keep the MSX driver's
...
def main():
    ...
    UDG_H.write_text(
        ...
        "\n"
        "/**\n"
        " * @brief 5 Card Stud UDGs for the Coleco Adam, TMS9918 GRAPHICS II\n"
        " * @verbose GENERATED by support/adam/convert-tiles.py from the MS-DOS\n"
        " *          CGA tiles in src/msdos/charset.h - do not edit by hand.\n"
        " *          1 bit per pixel; ink and paper colors are applied per cell\n"
        " *          by src/adam/graphics.c.\n"
        " */\n"
        "\n" + "\n".join(defines) + "\n"
        "\n"
    ...
    print(f"wrote {FONT_BIN} ({len(font)} bytes)")
    print(f"wrote {UDG_H} ({LAST_CODE - 0x80 + 1} glyphs, 0x80-0x{LAST_CODE:02X})")
